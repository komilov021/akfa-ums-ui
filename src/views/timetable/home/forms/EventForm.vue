  ...
  <div>
    ...
    >
      <div class="grid grid-cols-2 gap-4">
        <el-form-item label="Name of event" prop="name" class="w-full !mb-0">
          <el-input
...
const getTimes = ref([ruleForm.start_time, ruleForm.end_time]);
const roomStore = useRoomReferenceStore();
const staff = useStaffStore();
const options = ref<any[]>([]);
const grouplist = ref<any[]>([]);

const getstaff = computed(() => staff.getTeacherSelect);
const remoteMethod = (query: any) => {
  if (query) {
    options.value = getstaff.value.filter((item) => {
...
const timeChange = (val: any, type: any) => {
  if (val) {
    const formattedTime = dayjs(val, "H:mm").format("HH:mm");
    if (type === "start") {
      ruleForm.start_time = formattedTime;
    } else if (type === "end") {
      ruleForm.end_time = formattedTime;
    }
  ...
  await formData.validate(async (valid, fields) => {
    if (valid) {
      try {
        ...
        await timetableStore.fetchCreateTaskEvent(params);
