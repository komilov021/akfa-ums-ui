<template>
  <div class="h-full flex flex-col mt-2" v-loading="loading">
    ...
    <div class="flex items-center justify-between mt-2">
      ...
      <div class="flex items-center gap-4">
        <div>
          <el-button
            @click="openDialog(currentComponent?.id)"
            class="flex gap-2 items-center justify-center text-[#1F2A37]"
          >
            Appeal
            <span
              v-if="!components.has_appeal"
              class="bg-[#6B7280] text-white !w-8 !h-5 ml-2 rounded-[24px] flex justify-center items-center font-bold text-[10px]"
            >
              Off
            </span>
            <span
              v-else
              class="bg-[#00C851] text-white !w-8 !h-5 ml-2 rounded-[24px] flex justify-center items-center font-bold text-[10px]"
            >
              On
            </span>
          </el-button>
        ...
        <div class="flex items-center gap-2" v-if="!components.published">
          ...
          <el-button type="danger" @click="changeDeleteAll"
            >Delete All Grades</el-button
          >
          ...
          <el-dropdown @command="handleCommand">
            <el-button
              :disabled="
                !components.can_publish && !components.students?.length
              "
              >Publish</el-button
            >
            <template #dropdown>
              <el-dropdown-menu>
                <el-dropdown-item command="now">Publish now</el-dropdown-item>
                <el-dropdown-item
                  command="deadline-edit"
                  class="flex items-center justify-between gap-3"
                  v-if="currentDeadline && currentDeadline?.published_deadline"
                >
                  <div>
                    <p>Deadline</p>
                    <span>{{
                      formatDeadline(currentDeadline?.published_deadline)
                    }}</span>
                  </div>
                  <div
                    class="w-12 h-8 rounded-[32px] bg-[#F9FAFB] text-[#1F2A37] font-semibold text-xs justify-center flex items-center"
                    @click="openDeadlinePublish(currentDeadline?.id)"
                  >
                    Edit
                  </div>
                </el-dropdown-item>
                <el-dropdown-item command="deadline" v-else>
                  Publish with deadline
                </el-dropdown-item>
              </el-dropdown-menu>
            </template>
          </el-dropdown>
        ...
        <el-tag
          ...
          effect="dark"
          size="large"
          class="!bg-[#47CD89] !border-0 !cursor-pointer"
          >Published</el-tag
        >
      </div>
    </div>
  ...
  <el-dialog v-model="openPublishChangelog">
    <UpdateRequestDialog
      v-if="openPublishChangelog"
      @on-submit="publishGrade($event)"
    />
  </el-dialog>
  <el-dialog
    v-model="openAppealForm"
    :width="650"
    class="reset"
    :title="dialogTitle"
  >
    <AppealForm
      ...
      :type="appealType"
      :data="selectedAppeal"
      @on-submit="handleAppealSubmit($event)"
    />
  </el-dialog>
  <el-dialog v-model="publishDialog" title="Publish with deadline" width="500">
    <PublishDeadline
      v-if="publishDialog"
      :type="publishType"
      :data="selectDeadlinePublish"
      @on-submit="savePublishDeadline($event)"
    />
  </el-dialog>
</template>
...
import PublishDeadline from "./components/PublishDeadline.vue";
...
const currentCourse: any = computed(() => curriculumStore.currentCourse);
...
const generalType = ref("deleteAll");
const openPublishChangelog = ref(false);
const openAppealForm = ref(false);
const appealType = ref<"create" | "edit">("create");
const publishType = ref<"create" | "edit">("create");
const publishDialog = ref(false);
const staticRequest = ref({
  explanation: "Static request for improve",
  type: "",
  files: [
    {
      bucket_name: "2024-9",
...
const openDialog = (id: string) => {
  const appeal = currentCourse.value?.components?.find(
    (item: any) => item.id === id
  );
  if (appeal && appeal.has_appeal) {
    selectedAppeal.value = appeal;
    appealType.value = "edit";
    openAppealForm.value = true;
  } else {
    appealType.value = "create";
...
const formatDeadline = (deadline: string) => {
  if (!deadline) return "";
  return dayjs(deadline).format("D MMM / HH:mm");
};
const selectDeadlinePublish = ref<any>(null);
const currentDeadline = computed(() => {
  const component = currentCourse.value?.components?.find(
    (item: any) => item.id === route.params.component
  );
  return component;
});
console.log(currentDeadline, "currentDeadline");

const openDeadlinePublish = (id: string) => {
  const deadlineData = currentCourse.value?.components?.find(
    (item: any) => item.id === id
  );
  if (deadlineData?.published_deadline && deadlineData) {
    selectDeadlinePublish.value = deadlineData;
    publishType.value = "edit";
    publishDialog.value = true;
  } else {
    publishType.value = "create";
    selectDeadlinePublish.value = null;
    publishDialog.value = true;
  }
};
...
const savePublishDeadline = async (data: any) => {
  try {
    const formatterDeadline = dayjs(
      `${data.deadline_date} ${data.deadline_time}`,
      "YYYY-MM-DD HH:mm:ss"
    ).format("YYYY-MM-DD HH:mm:ss");
    const payload = {
      published_deadline: formatterDeadline,
      component_id: route.params.component as string,
      assigned_course_id: route.params.grade as string,
      retake: isRetake.value,
    };
    await gradeStore.publishGradeDeadline(payload);
    const successMessage =
      publishType.value === "create"
        ? "Deadline successfully published!"
        : "Deadline successfully updated!";

    ElMessage.success(successMessage);
    await curriculumStore.getAssignCourseById(route.params.grade as string);
  } catch (error) {
    ElMessage.error("An error occurred while saving the deadline.");
  }
...
const importTemplate = async () => {
  ...
  try {
    loading.value = true;
    staticRequest.value.type =
      RequestTypeEnum.component_grade_import_with_excel;
    await gradeStore.importMainGradingSystem({
      file_id: file.value.id,
      component_id: route.params.component as string,
      assigned_course_id: route.params.grade as string,
      retake: isRetake.value,
      request: staticRequest.value,
    });
    await fetchList();
...
const publishGrade = (request?: any) => {
    ...
    .then(async () => {
      try {
        ...
        const data = {
          ...
          retake: isRetake.value,
          request: staticRequest.value,
          published_deadline: currentDateTime,
        };
        data.request["type"] = data.is_published
          ? "publish_component_course_edit"
          : "unpublish_component_course_edit";
        // await gradeStore.publishGrade(data);
        await gradeStore.publishGradeDeadline(data);
        await fetchList();
        ElMessage({
          message: "Grade published",
          type: "success",
        });
        openPublishChangelog.value = false;
        loading.value = false;
      } catch (error: any) {
        console.log("error", error.message);
        loading.value = false;
...
const dialogTitle = computed(() =>
  appealType.value === "edit"
    ? "Edit Appeal’s Deadline"
    : "Create Appeal’s Deadline"
);

const handleAppealSubmit = async (data: any) => {
  try {
    const response = await curriculumStore.appealSetConfiguration({
      ...data,
      component_id: route.params.component as string,
      assigned_course_id: route.params.grade as string,
      retake: isRetake.value,
    ...
    ElMessage.success(
      appealType.value === "create" ? "Appeal created" : "Appeal updated"
    );
    openAppealForm.value = false;
    fetchList();
    await curriculumStore.getAssignCourseById(route.params.grade as string);
    if (response) {
      curriculumStore.currentAppeal = response;
      appealType.value = "edit";
    }
  } catch (error) {
...
const changeRequestImport = () => {
  generalType.value = "import";
  openRequestModal("component_grade_import_with_excel");
};
const changeDeleteAll = () => {
  generalType.value = "deleteAll";
  openRequestModal("component_grade_remove");
};
...
const openRequestModal = (type: any, id?: string) => {
  ...
  } else if (type === "component_grade_remove") {
    currentType.value = RequestTypeEnum.component_grade_remove;
  } else if (type == "component_grade_import_with_excel") {
    currentType.value = RequestTypeEnum.component_grade_import_with_excel;
  } else {
...
const deleteGrades = async (request: any) => {
  // ElMessageBox.confirm(
...
const submitForm = async (request: any) => {
  ...
  try {
    loading.value = true;
    // if (request.type == RequestTypeEnum.component_grade_import_with_excel) {
    //   importTemplate(request);
    // }
    // else
    if (request.type == RequestTypeEnum.component_grade_remove) {
      deleteGrades(request);
    } else if (request.type == RequestTypeEnum.course_grading_insert) {
