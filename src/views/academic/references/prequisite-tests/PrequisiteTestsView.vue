<template>
  <div class="mx-auto mt-6 md:mt-0">
    <h1 class="text-main font-bold text-lg">Prerequisite test types</h1>
    ...
    <div class="grid md:grid-cols-3 sm:grid-cols-2 gap-4">
      <div v-for="item of store.getTestTypesSelect" :key="item.value">
...
const i18n = useI18n();
const store = usePrequisiteTestsStore();
...
const confirmDelete = (id: string) => {
  ElMessageBox.confirm("This action cannot be undone. Continue?", "Warning", {
    ...
    .then(async () => {
      try {
        loading.value = true;
        const res = await store.removeTestTypes(id);
        console.log("Res", res);
