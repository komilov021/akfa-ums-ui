<template>
  <div class="mx-auto mt-6 md:mt-0">
    <h1 class="text-main font-semibold text-xl">Program degree types</h1>
...
const createDegree = async () => {
  ...
  ElMessage({
    message: `${degreeInput.value} has successfully created!`,
    type: "success",
    duration: 5 * 1000,
  });
  try {
...
const confirmDelete = (id: string) => {
  ElMessageBox.confirm("This action cannot be undone. Continue?", "Warning", {
    ...
    .then(async () => {
      try {
        loading.value = true;
        let response = await degreeStore.removeDegree(id);
