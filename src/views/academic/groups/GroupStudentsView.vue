  ...
  <div class="h-full flex flex-col mt-4 gap-4" v-loading="loading">
      ...
      >
        ...
        >
          <template #default="{ row }">
            <el-button
              class="action-button"
              @click="confirmStudentDelete(row.id)"
            >
              <el-icon size="20">
                <Delete />
              </el-icon>
    ...
    <teleport to="#modal" v-if="showModal">
      ...
      <GroupStudentDialog
        ...
        :semester="(route.params.semester as string)"
...
const file = ref();
const dialogType = ref<"import" | "student">();
...
const fetchList = () => {
  groupStore.fetchGroupStudents(filter);
};
...
const openImportModal = () => {
  ...
  modal.setTitle("Import students");
  modal.setWidth("30%");
  modal.showModal();
};
...
const importTemplate = async () => {
  ...
  try {
    await groupStore.importStudents({
...
const confirmStudentDelete = (id: string) => {
  ElMessageBox.confirm(
    ...
    .then(async () => {
      try {
        await groupStore.removeStudent(id);
        await fetchList();
        ElMessage({
          type: "success",
