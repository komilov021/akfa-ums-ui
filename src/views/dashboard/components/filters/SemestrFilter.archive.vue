<template>
  ...
  >
    <template #reference>
      <div
        class="border border-gray300 cursor-pointer bg-white rounded-lg flex items-center justify-center space-x-2 py-3 px-4"
      >
        <SvgIcon name="local-callendar" color="transparent" size="20" />
        <p class="text-base text-[#1B1B1B]">
          {{ fromSemestr?.name }} {{ fromSemestr?.year }} -
          {{ toSemestr?.name }}
          {{ toSemestr?.year }}
        </p>
      </div>
    ...
    <div class="px-8 flex flex-col space-y-4 semestr">
      <div class="space-y-2">
        ...
        <div class="flex items-center justify-center gap-2">
          <el-button
            ...
            :class="{
              '!text-primary':
                sem.row_number === statisticsStore.getFilters.semestrs?.from,
            }"
